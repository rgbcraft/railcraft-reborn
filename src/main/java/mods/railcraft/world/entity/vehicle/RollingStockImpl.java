...
public class RollingStockImpl implements RollingStock, INBTSerializable<CompoundTag> {
    ...
    public boolean maintainLink(Side linkSide) {
        ...
        var sameDimension = this.level().dimension().equals(linkedEntity.getLevel().dimension());
        ...
        var sub = new Vec2((float) (linkedEntity.getX() - this.minecart.getX()), (float) (linkedEntity.getZ() - this.minecart.getZ()));
        var unit = sub.equals(Vec2.ZERO) ? sub : sub.normalized(); // Check for NaN
        ...
        var dot = new Vec2((float) (linkedEntity.getDeltaMovement().x() - this.minecart.getDeltaMovement().x()), (float) (linkedEntity.getDeltaMovement().z() - this.minecart.getDeltaMovement().z())).dot(unit);

        var damping = highSpeed ? HS_DAMPING : DAMPING;
        var dampX = damping * dot * unit.x;
        var dampZ = damping * dot * unit.y;

        dampX = (float) limitForce(dampX);
        dampZ = (float) limitForce(dampZ);
