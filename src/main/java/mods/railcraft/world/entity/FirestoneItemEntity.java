...
public class FirestoneItemEntity extends ItemEntity {
    ...
    public void tick() {
        ...
        if (++this.clock % 4 == 0
            ...
            var player = (Entity) Util.mapNullable(owner, this.getLevel()::getPlayerByUUID);
            FirestoneItem.trySpawnFire(this.getLevel(), this.blockPosition(), getItem(), player);
        }
    }
    ...
    public void lavaHurt() {
        ...
        for (int i = 0; i < 10; i++) {
            ...
            if (LevelUtil.setBlockState(this.getLevel(), surface,
                    firestoneBlock.setValue(RitualBlock.CRACKED, cracked), player)) {
